...
def train_index(exp_dir1, version19="v2"):
    ...
    if big_npy.shape[0] > 2e5:
        print(f"Large dataset ({big_npy.shape[0]} features), applying k-means clustering to 10k centers...")
        try:
            config = Config()
            big_npy = (
                MiniBatchKMeans(
    ...
    for i in range(0, big_npy.shape[0], batch_size_add):
        ...
        if (i // batch_size_add) % 10 == 0:
            print(f"  Added {min(i + batch_size_add, big_npy.shape[0])}/{big_npy.shape[0]} features")
    ...
    symlink_path = "%s/%s_IVF%s_Flat_nprobe_%s_%s_%s.index" % (
        outside_index_root, exp_dir1, n_ivf, index_ivf.nprobe, exp_dir1, version19
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", required=True, help="Model/experiment name (e.g., 'kamel's voice')")
    parser.add_argument("--version", default="v2", choices=["v1", "v2"], help="Model version (v1 or v2)")
